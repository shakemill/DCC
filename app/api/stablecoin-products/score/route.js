...
Your task is to compute the Provider Quality Score (0–100) for each stablecoin product based on publicly available information.
...
/** POST /api/stablecoin-products/score – compute quality scores via ChatGPT and persist */
export async function POST(request) {
  try {
    ...
    const userPrompt = `Products to score (id | issuer | product | apy | jurisdiction | notes | sources | category):

${products
  .map(
    (p) =>
      `${p.id} | ${p.issuer ?? ''} | ${p.product ?? ''} | ${p.apy ?? ''} | ${p.jurisdiction ?? ''} | ${(p.notes ?? '').slice(0, 200)} | ${p.sources ?? ''} | ${p.category ?? ''}`
  )
  .join('\n')}
...
Return the JSON object with scores for each product id.`
    ...
    for (const entry of scores) {
      ...
      await prisma.stablecoinProduct.update({
        where: { id },
        data: {
    ...
    console.error('POST /api/stablecoin-products/score:', e)
    return NextResponse.json(
